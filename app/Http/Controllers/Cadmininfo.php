...
class Cadmininfo extends Controller
{
    ...
    public function updateProfile(Request $request)
    {
        ...
        $adminUser = Madminaccount::find($request->id);
    ...
    public function showAllAdmin()
    {
        ...
        $currentAdminId = auth()->id(); // Assuming you use Laravel's built-in auth system

        // Fetch all admins but exclude the currently logged-in admin
        $admin = Madminaccount::where('id', '!=', $currentAdminId)->get();

        // Check if there are no admins
        $noAdmin = $admin->isEmpty() ? "No Admin found" : null;

        return view('admin.admin_admin_user', compact('admin', 'noAdmin'));
    }

    public function changePassword(Request $request, $studentId)
    {
        $student = Madminaccount::find($studentId);
